...
class EpisodeStats:
    ...
    def add_step_info(self, reward, speed, distance, lane_deviation, waypoint_reached=False):
    ...
    def print_summary(self, episode_num, reason):
        ...
        logger.info(f"- Termination Reason: {reason}")
...
class CarlaRLEnv(gym.Env):
    ...
    def _on_collision(self, event):
        ...
        logger.info(f"⚠️ Collision detected with {event.other_actor.type_id}!")
    ...
    def _calculate_reward(self):
        """Simplified reward function similar to GitHub implementation but with traffic light handling"""
        reward = 0.0
        done = False
        ...
        speed = 3.6 * np.sqrt(vehicle_velocity.x**2 + vehicle_velocity.y**2 + vehicle_velocity.z**2)  # km/h
        ...
        # Stopped vehicle detection (except at red lights)
        at_red_light = False
        ...
        if speed < 1.0 and not at_red_light and self.episode_start_time + 10 < time.time():
            return -10.0, True
        
        # Speed limit violation
        if speed > params.OPTIMAL_SPEED_MAX + 10:
            return -10.0, True
        ...
        if self.vehicle.is_at_traffic_light():
            ...
            if light_state == carla.TrafficLightState.Red:
                if speed > 2.0:  # Moving at red light
                    reward -= 5.0
                else:  # Properly stopped
                    reward += 5.0
            elif light_state == carla.TrafficLightState.Yellow and speed > 15.0:
                reward -= 2.0  # Penalty for speeding through yellow
        ...
        if waypoint and waypoint.lane_type == carla.LaneType.Driving:
            ...
            distance_from_center = vehicle_location.distance(waypoint.transform.location)
            max_distance = 3.0  # Max acceptable distance from center
            
            # Calculate alignment with road
            alignment = vehicle_forward.x * waypoint_forward.x + vehicle_forward.y * waypoint_forward.y
            angle = np.arccos(max(min(alignment, 1.0), -1.0))
            max_angle = np.deg2rad(20)  # Max acceptable angle deviation
            
            # Reward factors (just like in GitHub implementation)
            centering_factor = max(1.0 - distance_from_center / max_distance, 0.0)
            angle_factor = max(1.0 - abs(angle / max_angle), 0.0)
            
            # Speed factor
            if speed < params.OPTIMAL_SPEED_MIN:
                speed_factor = speed / params.OPTIMAL_SPEED_MIN
            elif speed > params.OPTIMAL_SPEED_MAX:
                speed_factor = max(1.0 - (speed - params.OPTIMAL_SPEED_MAX) / 10.0, 0.0)
            else:
                speed_factor = 1.0
            
            # Combined reward (multiplicative approach)
            reward += 1.0 * centering_factor * angle_factor * speed_factor
        
        # Update waypoint progress (simplified)
        if self.target_waypoint:
            ...
            if hasattr(self, 'last_target_distance'):
                distance_improvement = self.last_target_distance - distance_to_target
                if distance_improvement > 0:
                    reward += distance_improvement * 0.5
            
            self.last_target_distance = distance_to_target
        
        # Get current control for implementing smooth steering
        control = self.vehicle.get_control()
        steering_angle = control.steer
        
        # Apply smoothing instead of jitter penalty
        if hasattr(self, 'last_steering_angle'):
            smooth_steering = self.last_steering_angle * 0.9 + steering_angle * 0.1
            self.vehicle.apply_control(carla.VehicleControl(
                throttle=control.throttle,
                steer=smooth_steering,
                brake=control.brake
            ))
        
        self.last_steering_angle = steering_angle
        
        # Termination if episode goes too long
    ...
    def _is_off_road(self):
        """Off-road detection that uses the None waypoint return value"""
        try:
            current_location = self.vehicle.get_location()
            # Use project_to_road=False to get actual waypoint at current location
            car_waypoint = self.world.get_map().get_waypoint(current_location, project_to_road=False)
            
            # If waypoint is None, vehicle is off-road
            if car_waypoint is None:
                #logger.info("Off-road detected: No waypoint found at vehicle location")
                return True
            
            # Otherwise, check if it's a driving lane
            is_driving = car_waypoint.lane_type == carla.LaneType.Driving
            if not is_driving:
                #logger.info(f"Off-road detected: Non-driving lane type: {car_waypoint.lane_type}")
                return True
            ...
            # If there's an exception, assume off-road
            return True
    ...
    def step(self, action):
        ...
        if sensor_data is None:
            return np.zeros(params.OUTPUT_DIM), 0, True, True, {"error": "Sensor data missing"}
        ...
        self.current_episode_stats.add_step_info(reward, speed, distance, lane_deviation, waypoint_reached)
        
        self.last_location = current_location
        
        # Determine if episode is done
        timeout = self.steps_taken >= self.max_steps
        done = self.collision_occurred or timeout or reward_done
    ...
    def reset(self, seed=None, options=None):
        ...
        try:
            ...
            self.world.tick()
            time.sleep(0.1)  # Wait for sensors to initialize
    ...
    def _on_step(self):
        ...
        if self.locals.get('dones') is not None:
            for done, info in zip(self.locals['dones'], self.locals['infos']):
                if done and info.get("termination_reason", None):
