...
def game_loop(args):
    ...
    world = None
    data = pd.DataFrame()
    #filename = 'deneme_1.h5' # model file path
    #loaded_model = load_model(filename) # model loader
    # DRIVER NAMES
    apo=0
    onder=0
    mehdi=0
    ...
    selin=0
    gonca=0
    loaded_model =  load_model('86_acc_cld.h5') # Model loading area
    try:
        ...
        while True:
            ...
            """
            low_data = {"accelX":world.imu_sensor.accelerometer[0],
            ...
            low_data = {"accelX":world.imu_sensor.accelerometer[0],
                        ...
                        "gyroZ":world.imu_sensor.gyroscope[2]}
            data = data.append(low_data , ignore_index = True)
            if x % 20 == 0: # The model input include 20 timesteps, 6 features, 1 batch
                data = np.array(data).reshape(1,20,6)
                out = loaded_model.predict(data)
                print(out)
                # THIS AREA IS TO CLASSIFY REPORT AND  COUNTER [APO,ONDER,HURCAN,MEHDI,GONCA,BERK,SELIN] are driver names.
                if  (np.argmax(out))== 0:
                    print("apo")
                    apo +=1
        ...
        # concat all counted drivers
        counter = np.array([apo,onder,hurcan,mehdi,berk,selin])
        ...
        print("Saved kardeSS")"""
        pygame.quit()
        # display the most frequent driver name
        print(counter)
        print(counter[np.argmax(counter)])
