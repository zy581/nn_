...
class Grasping_Module_multidiscrete(nn.Module):
    ...
    def forward(self, x, verbose=0):
        ...
        if verbose == 1:
            print("After Flatten: ".ljust(15), x.size())
        if self.output_activation is not None:
            return self.sigmoid(x)
        else:
            return x
...
def main():
    import cv2
    rgb = cv2.imread("grasprl/demo/rgb.jpg")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_trans = T.transforms.ToTensor()
    img_normalize = T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    tensor_rgb = img_trans(rgb).unsqueeze(0)
    tensor_rgb = img_normalize(tensor_rgb)
    tensor_depth = torch.randn((12,224,224))
    # input = torch.cat([tensor_rgb,tensor_depth],1)
    # q_net = MULTIDISCRETE_RESNET(1)
    # output = q_net(input).argmax().item()
    # max_idx = output.min()
    print(tensor_depth.argmax())
