...
class RandomCropComputer(Dataset):
    ...
    def five_crops(self, i, img):
        ...
        # Only squeeze the first dimension if its size is 1
        if img.shape[0] == 1:
        ...
        a = five_crop(img, self._get_size(img))
        return a
    ...
    def __init__(self, cfg, dataset_name, img_set, crop_type, crop_ratio):
        ...
        self.img_dir = os.path.join(self.save_dir, "imgs", img_set)
        self.label_dir = os.path.join(self.save_dir, "labels", img_set)
