...
class LitUnsupervisedSegmenter(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        img = batch["img"]
        label = batch["label"].squeeze(1)  # Remove the extra dimension if it exists
        self.net.eval()
        ...
        with torch.no_grad():
            ...
            # Obtain predictions from the linear probe and reshape for pixel-wise comparison
            linear_preds = self.linear_probe(code)
            linear_preds = linear_preds.argmax(dim=1)

            # Ensure preds and label have the same shape
            if linear_preds.shape != label.shape:
                raise ValueError(f"Shape mismatch: linear_preds shape {linear_preds.shape} != label shape {label.shape}")

            self.linear_metrics.update(linear_preds.view(-1), label.view(-1))

            cluster_loss, cluster_probs = self.cluster_probe(code, None)
            cluster_preds = cluster_probs.argmax(dim=1)

            # Ensure cluster_preds and label have the same shape
            if cluster_preds.shape != label.shape:
                raise ValueError(f"Shape mismatch: cluster_preds shape {cluster_preds.shape} != label shape {label.shape}")

            self.cluster_metrics.update(cluster_preds.view(-1), label.view(-1))
        ...
        with torch.no_grad():
            ...
            if self.trainer.is_global_zero and not self.cfg.submitting_to_aml:
                ...
                for i in range(self.cfg.n_images):
                    img = output["img"][i].squeeze()  # Remove extra dimensions if any
                    label = output["label"][i].squeeze()  # Remove extra dimensions if any
                    linear_preds = output["linear_preds"][i].squeeze()  # Remove extra dimensions if any
                    cluster_preds = output["cluster_preds"][i].squeeze() 
                    ax[0, i].imshow(prep_for_plot(img))
                    ax[1, i].imshow(self.label_cmap[label])
                    ax[2, i].imshow(self.label_cmap[linear_preds])
                    ax[3, i].imshow(self.label_cmap[self.cluster_metrics.map_clusters(cluster_preds)])
