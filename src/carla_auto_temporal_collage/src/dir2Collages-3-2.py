...
output_dir = 'data/collages/collages-3fps-3-2'
    ...
    if os.path.isdir(subfolder_path):
        ...
        for inner_subfolder in os.listdir(subfolder_path):
            ...
            if os.path.isdir(inner_subfolder_path):
                ...
                num_collages = len(frame_files) // 6 + (1 if len(frame_files) % 6 > 0 else 0)
                
                # Determine the padding width based on the number of collages
                padding_width = len(str(num_collages))

                # Loop through each set of frames (up to 6 frames)
                for collage_index in range(num_collages):
                    # Get the current set of frames (up to 6 frames)
                    frames = []
                    for i in range(6):
                        if collage_index * 6 + i < len(frame_files):
                            frame_path = os.path.join(inner_subfolder_path, frame_files[collage_index * 6 + i])
                    ...
                    if frames:
                        ...
                        padding = 15
                        collage_height = 3 * frame_height + 4 * padding
                        ...
                        positions = [
                            ...
                            (2 * padding + frame_height, padding),  # Middle-left
                            (2 * padding + frame_height, 2 * padding + frame_width),  # Middle-right
                            (3 * padding + 2 * frame_height, padding),  # Bottom-left
                            (3 * padding + 2 * frame_height, 2 * padding + frame_width)  # Bottom-right
                        ]
                        ...
                        for i, (frame, pos) in enumerate(zip(frames, positions)):
                            ...
                            cv2.putText(
                                collage,
                                f'{collage_index * 6 + i + 1}',  # Frame number
                                (x + 10, y + 40),  # Position
