...
class ROSCmdVelHandler(threading.Thread):
    ...
    def _cmd_vel_callback(self, msg):
        ...
        if hasattr(self.stabilizer, "_should_log") and self.stabilizer._should_log("ros_cmd_vel", 0.5):
            print(f"[ROS指令] 速度={target_speed:.2f} | 转向={target_turn:.2f}rad | 当前步态: {self.stabilizer.gait_mode}")
...
class HumanoidStabilizer:
    def __init__(self, model_path):
        ...
        self.base_kp_knee = 300.0  # 强化膝盖
        self.base_kd_knee = 50.0
        self.base_kp_ankle = 300.0 # 强化脚踝
        self.base_kd_ankle = 60.0
        self.kp_waist = 200.0      # 强化腰部，防软腰
        self.kd_waist = 40.0
        self.kp_arm = 20.0
        self.kd_arm = 20.0

        # 重心目标调整，保持自然垂直
        self.com_target = np.array([0.0, 0.0, 0.82])
        self.kp_com = 50.0
        ...
        self.com_safety_threshold = 0.5  
        self.speed_reduction_factor = 0.5  
        self._support_duration = 5.0
        self._support_com_z = 0.78
        torso_body_id = int(mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, "torso"))
        pelvis_body_id = int(mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, "pelvis"))
        if torso_body_id >= 0: self._support_body_id = torso_body_id
        elif pelvis_body_id >= 0: self._support_body_id = pelvis_body_id
        else: self._support_body_id = 0
        ...
        self.state = "STAND" 
        self.state_map = {
            "STAND": self._state_stand, "WALK": self._state_walk,
            "STOP": self._state_stop, "EMERGENCY": self._state_emergency 
        }
    ...
    def _init_stable_pose(self):
        ...
        # ==================== 【关键修改 2】纠正姿态，改为笔挺站姿 ====================
        # 原版是膝盖-0.4，导致重心严重偏前。改为直立(0.0)让重力自然贯穿腿骨
        ...
        # 基于全新的直立姿态捕捉悬空辅助力的高度目标
        self._support_com_z = float(self.data.subtree_com[0][2])
        self._support_until = float(self.data.time) + float(self._support_duration)
    ...
    def _simulate_imu_data(self):
        ...
        self.imu_data_buffer.append({
            "euler": noisy_euler, "ang_vel": noisy_ang_vel,
            "true_euler": true_euler, "true_ang_vel": true_ang_vel
        })
    ...
    def _simulate_foot_force_data(self):
        ...
        if now_t < float(self._support_until):
            left_contact = 1 if true_left_force > 1.0 else 0
            right_contact = 1 if true_right_force > 1.0 else 0
            self.foot_data_buffer.clear()
            return {"left_force": true_left_force, "right_force": true_right_force,
                    "left_contact": left_contact, "right_contact": right_contact,
                    "true_left_force": true_left_force, "true_right_force": true_right_force}
        ...
        self.foot_data_buffer.append({
            "left_force": noisy_left_force, "right_force": noisy_right_force,
            "left_contact": left_contact, "right_contact": right_contact,
            "true_left_force": true_left_force, "true_right_force": true_right_force
        })
    ...
    def _get_sensor_data(self):
        if not self.enable_sensor_simulation:
            imu_data = {"euler": self._get_root_euler(), "ang_vel": self.data.qvel[3:6].astype(np.float64).copy(),
                        "true_euler": self._get_root_euler(), "true_ang_vel": self.data.qvel[3:6].astype(np.float64).copy()}
            self._detect_foot_contact()
            foot_data = {"left_force": self.left_foot_force, "right_force": self.right_foot_force,
                         "left_contact": self.foot_contact[1], "right_contact": self.foot_contact[0],
                         "true_left_force": self.left_foot_force, "true_right_force": self.right_foot_force}
        else:
            imu_data = self._simulate_imu_data()
            foot_data = self._simulate_foot_force_data()

        self.current_sensor_data = {"imu": imu_data, "foot": foot_data, "time": self.data.time, "gait_mode": self.gait_mode}
        return self.current_sensor_data

    def print_sensor_data(self):
        if not self.current_sensor_data: return
        imu = self.current_sensor_data["imu"]
        foot = self.current_sensor_data["foot"]
        print("\n=== 传感器数据 ===")
        print(f"仿真时间: {self.current_sensor_data['time']:.2f}s | 模拟状态: {'开启' if self.enable_sensor_simulation else '关闭'} | 当前步态: {self.gait_mode}")
        print(f"IMU欧拉角(roll/pitch/yaw): {imu['euler'][0]:.3f}/{imu['euler'][1]:.3f}/{imu['euler'][2]:.3f}rad")
        print(f"左脚力: {foot['left_force']:.2f}N | 接触: {foot['left_contact']}")
        print(f"右脚力: {foot['right_force']:.2f}N | 接触: {foot['right_contact']}")
        print("==================\n")
    ...
    def _state_walk(self):
        ...
        right_hip_offset = self.right_leg_cpg.update(self.dt, target_phase=self.left_leg_cpg.phase, speed_factor=speed_factor, turn_factor=turn_factor)
        left_hip_offset = self.left_leg_cpg.update(self.dt, target_phase=self.right_leg_cpg.phase, speed_factor=speed_factor, turn_factor=turn_factor)
        ...
        if self.gait_mode != "STEP_IN_PLACE":
            self.joint_targets[self.joint_name_to_idx["abdomen_y"]] = 0.0 # 保持腰部挺直，避免因速度前倾

        # 以0.0为基准线的步态偏移
        self.joint_targets[self.joint_name_to_idx["hip_y_right"]] = 0.0 + right_hip_offset
        self.joint_targets[self.joint_name_to_idx["knee_right"]] = 0.0 - right_hip_offset * 1.2
        ...
        self.joint_targets[self.joint_name_to_idx["knee_left"]] = 0.0 - left_hip_offset * 1.2
    ...
    def set_state(self, state):
        if state in self.state_map.keys():
            if state == "WALK" and float(self.data.time) < float(self._recovery_until):
                return
            self.state = state
            if state == "WALK": self.walk_start_time = None
            elif state == "STAND": self._init_stable_pose()
    ...
    def _calculate_stabilizing_torques(self):
        ...
        roll_torque = (self.kp_roll * stand_scale) * roll_error + (self.kd_roll * stand_scale) * (-self._imu_angvel_filt[0]) + 5.0 * self.integral_roll
        ...
        pitch_torque = (self.kp_pitch * stand_scale) * pitch_error + (self.kd_pitch * stand_scale) * (-self._imu_angvel_filt[1]) + 4.0 * self.integral_pitch

        yaw_error = -self._imu_euler_filt[2]
        yaw_torque = (self.kp_yaw * stand_scale) * yaw_error + (self.kd_yaw * stand_scale) * (-self._imu_angvel_filt[2])

        torso_torque = np.array([roll_torque, pitch_torque, yaw_torque])
        torso_torque = np.clip(torso_torque, -50.0, 50.0)

        # 维持你原版代码里的精妙辅助力，帮助机器人度过初始化
        self.data.xfrc_applied[self._support_body_id, :] = 0.0
        now_t = float(self.data.time)
        if now_t < float(self._support_until):
            scale = float(np.clip((float(self._support_until) - now_t) / float(self._support_duration), 0.0, 1.0))
            com_z = float(self.data.subtree_com[0][2])
            z_error = float(self._support_com_z - com_z)
            z_vel = float(self.data.qvel[2])
            baseline = self.weight * 0.15
            up_force = float(np.clip(baseline + 2000.0 * z_error - 200.0 * z_vel, 0.0, self.weight * 1.2))
            self.data.xfrc_applied[self._support_body_id, 2] = up_force * (scale * scale)
            self.data.xfrc_applied[self._support_body_id, 3] = (-120.0 * self._imu_euler_filt[0] - 30.0 * self._imu_angvel_filt[0]) * scale
            self.data.xfrc_applied[self._support_body_id, 4] = (-120.0 * self._imu_euler_filt[1] - 30.0 * self._imu_angvel_filt[1]) * scale

        # ==================== 【关键修改 3】剔除错误的反馈环 ====================
        # 只保留最纯粹、最可靠的刚性 PD 跟踪，让它强硬地维持设定好的直立形态
        ...
        for joint_name in leg_joints:
            ...
            if self.enable_robust_optim and self.state == "WALK":
                if "right" in joint_name: force_factor = np.clip(self.right_foot_force / self._force_factor_norm, 0.4, 1.1)
                else: force_factor = np.clip(self.left_foot_force / self._force_factor_norm, 0.4, 1.1)
            else:
                force_factor = 1.0  

            if "hip" in joint_name:
                kp = self.base_kp_hip * force_factor * stand_scale
                kd = self.base_kd_hip * force_factor * stand_scale
            elif "knee" in joint_name:
                kp = self.base_kp_knee * force_factor * stand_scale
                kd = self.base_kd_knee * force_factor * stand_scale
            elif "ankle" in joint_name:
                kp = self.base_kp_ankle * force_factor * stand_scale
                kd = self.base_kd_ankle * force_factor * stand_scale
        ...
        # 足够撑起躯干
        torque_limits = {
            "abdomen_z": 100, "abdomen_y": 200, "abdomen_x": 100,
            "hip_x_right": 200, "hip_z_right": 150, "hip_y_right": 200,
            "knee_right": 300, "ankle_y_right": 200, "ankle_x_right": 100,
            "hip_x_left": 200, "hip_z_left": 150, "hip_y_left": 200,
            "knee_left": 300, "ankle_y_left": 200, "ankle_x_left": 100,
            "shoulder1_right": 40, "shoulder2_right": 40, "elbow_right": 40,
            "shoulder1_left": 40, "shoulder2_left": 40, "elbow_left": 40
        }
        ...
        if self.data.time > self.init_wait_time and self._should_log("walk_debug", 2.0):
            print(f"=== 行走调试 ===")
            print(f"状态: {self.state} | 步态: {self.gait_mode} | 速度: {self.walk_speed:.2f}")
            print(f"右脚接触: {self.foot_contact[0]}, 左脚接触: {self.foot_contact[1]}")

        self.prev_com = self.data.subtree_com[0].copy()
        return torques
            ...
            with viewer.launch_passive(self.model, self.data) as v:
                ...
                while self.data.time < self.sim_duration:
                    ...
                    if self._should_log("status", 2.0):
                        ...
                        print(f"时间:{self.data.time:.1f}s | 重心(z):{com[2]:.3f}m | 姿态(pitch):{euler[1]:.3f}rad | 脚接触:{self.foot_contact}")
                    ...
                    tilt = max(abs(euler_for_fall[0]), abs(euler_for_fall[1]))
                    if com[2] < 0.25 or (com[2] < 0.4 and tilt > 0.6):
                        self._fall_count += 1
                        print(f"跌倒！#{self._fall_count} 时间:{self.data.time:.1f}s | 重心(z):{com[2]:.3f}m | 最大倾角:{tilt:.3f}rad")
